...
void process_b(int msgid1, int msgid2, int pid_c){
    ...
    while(1){
        ...
        for(i=0; i<SIZE; i++){ /* Realizamos cambios */
            if(msg_rcv.info.mtext[i] == 'z') msg_rcv.info.mtext[i] = 'a';
            else if(msg_rcv.info.mtext[i] < 'z' && msg_rcv.info.mtext[i] >= 'a') msg_rcv.info.mtext[i] = msg_rcv.info.mtext[i]+1;
        ...
        memset(msg_rcv.info.mtext,0,sizeof(msg_rcv));
...
void process_c(int msgid, char *filename){
    ...
    while(1){
        ...
        if((check = msgrcv(msgid, (struct msgbuf *)&msg_rcv, (SIZE), 2, 0)) == -1){
          ...
          fprintf(f,"%s",msg_rcv.info.mtext);
        }
